...
import { draftStore, editorState } from "@/webmcp/context-store";
...
export function NoteForm({ action, workspaces, note, submitLabel }: Props) {
  ...
  // エージェントの draft_note で流し込まれた下書き（新規作成のときだけ）
  const draft = useSyncExternalStore(draftStore.subscribe, draftStore.get, () => null);
  const initial = !note && draft ? draft : null;
  useEffect(() => () => editorState.setDirty(false), []);
  return (
    <form
      key={initial?.id ?? "form"}
      action={formAction}
      className="stack"
      aria-label="ノートの編集"
      onChange={() => editorState.setDirty(true)}
      onSubmit={() => {
    ...
    >
      ...
      <label>
        タイトル
        <input name="title" defaultValue={note?.title ?? initial?.title} />
      </label>
      <label>
        本文（Markdown）
        <textarea name="body" rows={16} defaultValue={note?.body ?? initial?.body} />
      </label>
      {state.error && (
